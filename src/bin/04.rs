...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    for (y, l) in mat.iter().enumerate() {
        for (x, c) in l.iter().enumerate() {
            if *c == b'X' {
                let mut new_words = Word::candidates_from(x as u32, y as u32).into_iter().filter(|w| w.valid(&mat)).collect();
...
impl Cross {
    fn from_mat(x:u32, y:u32, mat: &[&[u8]]) -> Option<Cross> {
        let get = |xoff, yoff| mat.get(y.checked_add_signed(yoff)?  as usize)?.get(x.checked_add_signed(xoff)? as usize);
        if mat[y as usize][x as usize] != b'A' {return None};
        match get(-1, -1)? {
            b'M' => if *get(1,1)? != b'S' {return None},
            b'S' => if *get(1,1)? != b'M' {return None},
            _ => return None,
        };
        match get(1,-1)? {
            b'M' => if *get(-1,1)? != b'S' {return None},
